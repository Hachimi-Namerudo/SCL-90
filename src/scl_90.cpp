...
void SCL_90::calculate()//写完这个就去找纱世里去。这次要抱得紧一点，因为有早八。
{
    ...
    ui->avr->setText(QString::number((totalScore / 90),'f',2));
    for(int i = 0;i < 10;i++)
    {
        ui->PointTable->setItem(i,0,new QTableWidgetItem(QString::number(dimScore[i],'f',2)));
        ui->PointTable->setItem(i,1,new QTableWidgetItem(QString::number(dimScore[i] / arg[i],'f',2)));
    }
    ui->PointTable->setItem(10,0,new QTableWidgetItem(QString::number(totalScore)));
    ui->PointTable->setItem(10,1,new QTableWidgetItem(QString::number((totalScore / 90),'f',2)));
